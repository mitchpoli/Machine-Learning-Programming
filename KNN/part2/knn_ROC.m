...
%Initialization of the arrays TP_rate and FP_rate
TP_rate = zeros(1,K); 
FP_rate = zeros(1,K);

%For loop that calculates A for each value of k
for i=1:K
    ...
    %Calculate the matrix C and extrapolate its variables
    ...
    TP_rate(i) = TP / (TP+FN); %TPR for the current value of k
    FP_rate(i) = FP / (FP+TN); %FPR for the current value of k
end
