...
%For loop executed for each cluster
for i=1:K
    likelihood = gaussPDF(X, Mu(:, i), Sigma(:,:, i));  %Execute the funtion gaussPDF
    Pk_x(i,:) = Priors(i)*likelihood;  %Multiplies the obtained likelihood by the value of the prior corresponding to that cluster
end

p_tot = sum(Pk_x, 1);  %Sum the various Pk_x obtained previously
Pk_x = Pk_x./p_tot;   %Divides each Pk_x value by the total sum of Pk_x
